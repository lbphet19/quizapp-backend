...
//		Stream<AnswerRequestDTO> answerStream = Stream.of(answers);
	...
	public ResponseEntity<AnswerResponseDTO> postAnswers(
			...
			@RequestParam(name = "attemptId",defaultValue = "0") int attemptId){
		int totalScore = 0;
...
//		List<UserExamScore> userExamScore = userExamRepo.findUserLastAttempt(examId, userDetailsImpl.getId());
		UserExamScore userExamScore = userExamRepo.findById(attemptId).get();
		userExamScore.setStatus("finished");
		userExamScore.setScore(totalScore+"/"+correctAnswers.size());
		userExamRepo.save(userExamScore);
